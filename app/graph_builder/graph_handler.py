...
from graph_builder.vpc_graph import graph_vpc
from graph_builder.subnet_graph import graph_subnet
from graph_builder.rt_graph import graph_route_table
...
from graph_builder.instance_profile_graph import graph_instance_profile
...
def run_graph_builder(collected: Dict[str, Any], normalized_map: Dict[str, Any]) -> Dict[str, Any]:
    ...
    normalized_map["edges"].extend(graph_vpc(collected, account_id, region))
    normalized_map["edges"].extend(graph_subnet(collected, account_id, region))
    normalized_map["edges"].extend(graph_route_table(collected, account_id, region))
    ...
    normalized_map["edges"].extend(graph_instance_profile(collected, account_id, region))
